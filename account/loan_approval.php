...
$stmt = $pdo->query("SELECT a.*, b.name_en, b.name_bn, b.mobile, c.product_name FROM loan_application a LEFT JOIN members_info b ON a.member_id = b.id LEFT JOIN loan_info c ON a.product_code = c.product_code WHERE a.status = 'I' ORDER BY a.id DESC");
$loanApprovals = $stmt->fetchAll(PDO::FETCH_ASSOC);

if ($method === 'POST' && isset($_POST['loan_id'], $_POST['status'])) {
    $loan_id = (int)$_POST['loan_id'];
    $status = in_array($_POST['status'], ['P', 'R', 'I']) ? $_POST['status'] : 'P';
    ...
    if ($stmt->rowCount()) {
        if ($status === 'P') {
            $_SESSION['success_msg'] = '⏳ Loan approval is pending review.';
        } elseif ($status === 'I') {
            $_SESSION['success_msg'] = '⚠️ Loan approval set to inactive.';
        } else {
                                            ...
                                            <form method="post" class="flex-grow-1">
                                                <div class="d-flex align-items-center gap-1">
                                                    ...
                                                    <select name="status" class="form-select form-select-sm">
                                                        <option value="P" <?php echo $loan['status'] === 'P' ? 'selected' : ''; ?>>Initial Approve</option>
                                                        <option value="I" <?php echo $loan['status'] === 'I' ? 'selected' : ''; ?>>Inactive</option>
