...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_method === 'admission' && $amount > 0) {
        ...
        // Calculate sundry_share (remaining balance after payment)
        // This is the value from total_share_value field after amount deduction
        $sundry_share = $total_share_value;
        ...
        $stmt = $pdo->prepare("UPDATE member_share SET share_amount = share_amount + ?, other_fee = other_fee + ?, sundry_share = ? WHERE member_id = ? AND member_code = ?");
        $stmt->execute([$share_amount, $other_fee, $sundry_share, $member_id, $member_code]);
